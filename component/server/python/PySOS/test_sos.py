...
class SOSRequest(XMLRequest):
  ...
  def FetchXML(self):
    ...
    if self.IsException():
      raise SOSException(self.AsText())    

class GetCapabilities(SOSRequest):
  def __init__(self, rooturl):
    url = rooturl + "?request=GetCapabilities"
    SOSRequest.__init__(self, url)

  def GetOfferings(self):
    return self.GetValues('//sos:ObservationOffering/gml:name/text()')

  def GetOfferingsContaining(self, pattern):
    return self.GetValues('//sos:ObservationOffering[contains(gml:description/text(),"%s")]/gml:name/text()' % (pattern,))

  def GetObservedProperties(self, offering):
    offpath = '//sos:ObservationOffering[gml:name/text()="%s"]' % (offering,)
...
class DescribeSensor(SOSRequest):
  def __init__(self, sensorid, rooturl):
    ...
    url = rooturl + "?request=DescribeSensor&SystemId=%s" % (Quote(sensorid),)
    SOSRequest.__init__(self, url)
...
class GetObservation(GetObservation):
  ...
  def FetchXML(self):
    ...
    self.url = self.rooturl + "?" + "&".join(conds)
    print self.url
...
def main():
  ...
  x = time.time()
  r.FetchXML()
  print "fetched in %s seconds" % (time.time() - x,)
  sys.stdout.write("success\n")
  print r.GetOfferings()
